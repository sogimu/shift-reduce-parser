...
TEST( NAIVE_STACK_INTERPRETER_CFG_POSITIVE_SYNTAX_TREE_SCOPE, DISABLED_F_SEMICOLON_F_SEMICOLON )
{
   // ARRANGE
   const auto& input = R"""(
    {
   ...
   AbstractSyntaxTree syntax_tree { scope };
   ...
   const auto& expected_scope = std::make_shared< ScopeSyntaxNode >( std::vector< ISyntaxNodeSP > {expected_statment0, expected_statment1}, expected_scope_lexical_tokens );
   
   ControlFlowGraph expected_cfg{ expected_scope };
