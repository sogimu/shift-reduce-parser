...
class Entity
{
    ...
    std::vector<EntitySP>::iterator begin() { return mChildren.begin(); }
    std::vector<EntitySP>::iterator end() { return mChildren.end(); }

    std::string Name() const { return mName; }

    void Append( const EntitySP& entity ) { mChildren.push_back( entity ); }
...
TEST_CASE( "DFS pre-order range", "[wef]" )
{
    ...
    const auto& r = std::mismatch(
        range0.begin(), range0.end(), range1.begin(), range1.end(), []( const auto& a, const auto& b ) -> bool {
