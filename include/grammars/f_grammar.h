...
public:
    F()
    {
        ...
        mProductions.emplace_back([this](const Stack& stack) -> std::optional<Plan>
        {
            ...
            iterate_over_last_n_nodes(stack, 2, SyntaxNodeEmptyVisitor::Handlers{
                                          .default_handler = [&state](const ISyntaxNodeSP&)
                                          {
                                              state = State::ERROR;
                                          },
                                          .minus_syntax_node = [&minus, &number, &state](const MinusSyntaxNodeSP& node)
                                          {
                                          ...
                                          .number_syntax_node = [&number, &state](const NumberSyntaxNodeSP& node)
                                          {
        ...
        mProductions.emplace_back([this](const Stack& stack) -> std::optional<Plan>
        {
            ...
            iterate_over_last_n_nodes(stack, 1, SyntaxNodeEmptyVisitor::Handlers
                                      {
                                          .default_handler = [&state](const ISyntaxNodeSP&)
                                          {
                                              state = State::ERROR;
        ...
        mProductions.emplace_back([this](const Stack& stack) -> std::optional<Plan>
        {
            ...
            iterate_over_last_n_nodes(stack, 3, SyntaxNodeEmptyVisitor::Handlers{
                                          .default_handler = [&state](const ISyntaxNodeSP&)
                                          {
                                              state = State::ERROR;
                                          },
                                          .minus_syntax_node = [&first_minus, &second_minus, &number, &state](const MinusSyntaxNodeSP& node)
                                          {
                                              if( state == State::START )
                                              {
                                                  state = State::FIRST_MINUS;
...
//        mProductionByFeature.emplace_back(Features{one(Token_Type::OPEN_CIRCLE_BRACKET), one(Token_Type::MINUS), one(Token_Type::INT)}, [this](const Stack& stack) -> Production
//        {
...
//        mProductionByFeature.emplace_back(Features{one(Token_Type::MULTIPLY), one(Token_Type::MINUS), one(Token_Type::INT)}, [this](const Stack& stack) -> Production
//        {
...
//        mProductionByFeature.emplace_back(Features{one(Token_Type::DIVIDE), one(Token_Type::MINUS), one(Token_Type::INT)}, [this](const Stack& stack) -> Production
//        {
...
//        mProductionByFeature.emplace_back(Features{one(Token_Type::INT)}, [this](const Stack& stack) -> Production
//        {
//            auto it = stack.begin();
...
//        mProductionByFeature.emplace_back(Features{one(Token_Type::BOL), one(Token_Type::F), one(Token_Type::EOL)}, [this](const Stack& stack) -> Production
//        {
//            auto it = stack.begin();
//            std::advance(it, stack.size() - 3);
//            const auto& bol = *it;
