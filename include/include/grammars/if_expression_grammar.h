   ...
   If()
   {
      ...
      mProductions.emplace_back(
         [ this ]( const Stack& stack ) -> std::optional< Plan >
         {
            ...
            const auto& scope_node = std::make_shared< ScopeSyntaxNode >();
            scope_node->Add( conditional_expression );
            const auto& if_expression_node = std::make_shared< IfExpressionSyntaxNode >( conditional_expression, scope_expression );
            // if_expression_node->Add( scope_expression );
            scope_node->Add( if_expression_node );

            plan.to_add.nodes.push_back( /* if_expression_node */ scope_node );
            return plan;
         } );
   }
};
