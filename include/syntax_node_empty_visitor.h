...
public:
    struct Handlers
    {
        ...
        std::function<void(const ESyntaxNodeSP&)> e_syntax_node = [this](const ESyntaxNodeSP& node) { default_handler(node); };
        ...
        std::function<void(const OpenCurlyBracketSyntaxNodeSP&)> open_curly_bracket_syntax_node = [this](const OpenCurlyBracketSyntaxNodeSP& node) { default_handler(node); };
        std::function<void(const CloseCurlyBracketSyntaxNodeSP&)> close_curly_bracket_syntax_node = [this](const CloseCurlyBracketSyntaxNodeSP& node) { default_handler(node); };
        std::function<void(const OpenCircleBracketSyntaxNodeSP&)> open_circle_bracket_syntax_node = [this](const OpenCircleBracketSyntaxNodeSP& node) { default_handler(node); };
        std::function<void(const CloseCircleBracketSyntaxNodeSP&)> close_circle_bracket_syntax_node = [this](const CloseCircleBracketSyntaxNodeSP& node) { default_handler(node); };
        std::function<void(const ComputationalExpressionSyntaxNodeSP&)> computational_expression_syntax_node = [this](const ComputationalExpressionSyntaxNodeSP& node) { default_handler(node); };
        std::function<void(const ConditionalExpressionSyntaxNodeSP&)> conditional_expression_syntax_node = [this](const ConditionalExpressionSyntaxNodeSP& node) { default_handler(node); };
        std::function<void(const PrintExpressionSyntaxNodeSP&)> print_expression_syntax_node = [this](const PrintExpressionSyntaxNodeSP& node) { default_handler(node); };
        std::function<void(const VaribleAssigmentSyntaxNodeSP&)> varible_assigment_syntax_node = [this](const VaribleAssigmentSyntaxNodeSP& node) { default_handler(node); };
    ...
    {
        mHandlers.expression_syntax_node(node);
    }

    void visit(const ScopeSyntaxNodeSP& node) override
    {
        mHandlers.scope_syntax_node(node);
    }
    ...
    {
        mHandlers.print_expression_syntax_node(node);
    }

    void visit(const VaribleAssigmentSyntaxNodeSP& node) override
    {
        mHandlers.varible_assigment_syntax_node(node);
    }
