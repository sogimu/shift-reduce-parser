                                   ...
                                   const ComputationalExpressionSyntaxNodeSP& expression )
{
    ...
    iterative_dfs<ISyntaxNodeSP>(
        ...
        [&varible_store, &syntax_node_value_by_syntax_node]( const ISyntaxNodeSP& node ) -> bool {
            ...
            handlers.name_syntax_node = [&varible_store,
                                         &syntax_node_value_by_syntax_node]( const NameSyntaxNodeSP& name ) {
...
void HandleExpression( const VaribleStore& varible_store, const ExpressionSyntaxNodeSP& expression )
{
    ...
    handlers.computational_expression_syntax_node =
        [&varible_store]( const ComputationalExpressionSyntaxNodeSP& node ) {
...
double Calculator::solve( const std::string& expression ) const
{
    ...
    iterative_managed_dfs(
        ...
        [&varible_store]( const ISyntaxNodeSP& node ) -> std::optional<std::vector<ISyntaxNodeSP>> {
            ...
            handlers.print_expression_syntax_node = [&varible_store,
                                                     &node]( const PrintExpressionSyntaxNodeSP& print_expression ) {
            ...
            handlers.varible_assigment_syntax_node = [&varible_store,
                                                      &node]( const VaribleAssigmentSyntaxNodeSP& varible_assigment ) {
